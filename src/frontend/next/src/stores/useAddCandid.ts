...
const urlSlice: StateCreator<AddCandidStore, [], [], UrlSlice> = (
  ...
  lookupUrl: async () => {
    try {
      ...
      const isNameInstack = (name: string, stack: Tech[]) => {
        let i = -1;
      ...
      const actualStack: TechCreate[] = [];

      stack.forEach((techName) => {
        let index = isNameInstack(techName, foundStack);
        if (index != -1) actualStack.push(foundStack[index]);
        else actualStack.push({ name: techName, id: null });
      });
      ...
      const cp: Company = await getOrCreateCompanyByName(
